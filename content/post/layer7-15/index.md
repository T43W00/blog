...
디컴파일된 코드를 보면 플래그를 동적으로 생성하고, 사용자에게 입력받은 값을 비교하는 것을 확인할 수 있다.
...
코드를 보면 switch-case문을 통해 `a3 % 8`에 해당하는 case로 분기하는 것을 알 수 있는데, `output.txt`가 39byte이므로, `case 7`(78 % 8)으로 분기하는 것을 알 수 있다.

<br>

따라서 입력받은 값에 `case 7`부터 시작해서 순서대로 `[0x66, 0x44, 0x11, 0x77, 0x55, 0x22, 0x33, 0x88]` 값들과 xor 연산을 해주는 것이다.
...
압축되거나 암호화된, 즉 Packed 파일에서 동적으로 원래 파일로 압축을 해제하거나 복호화하여 실행하는 것.
...
`x32`로 계속해서 한 줄씩 실행하다 보면, 위와 같이 다른 주소로 실행 흐름이 바뀌는 것을 확인할 수 있다.
...
```py
bytes = [120 ^ 0x34, 49, 124 ^ 0x32, (-35 ^ 0x88) & 0xFF, 88, 1]
