...
중간에 인자의 순서가 바뀌는 이유는 `hanoi(n - 1, from, to, via);` 코드의 경우에는 원판이 홀수일 때와 짝수일 때 `n - 1`개의 원판을 옮겨야 하는 목적지 기둥이 2번째와 3번째로 각각 다르기 때문이다.

`hanoi(n - 1, via, from, to);` 코드는 `n - 1`개의 원판을 중간 기둥으로 옮기고 난 뒤에 중간 기둥에서 목적지 기둥으로 옮겨야 하기 때문에 순서가 바뀌었다.
...
입력받은 `N`의 크기 만큼 메모리를 할당하여 배열을 선언하였다. 선택 정렬 알고리즘을 통해 크기 순으로 정렬하고, 반복문을 통해 `i`번째 값과 `i + 1`번째 값이 다른 경우만 출력되게 하여 중복을 방지하였다.
